...
Primary entrypoint: validate_spec().
"""
...
def validate_spec(path: Path) -> Dict[str, Any]:
    """Validate a CLI spec file and return a decision_trace payload."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "CLI spec validation output (non_operational_output). "
            "Both cli_spec_validator.py entrypoints delegate here."
        ),
    )
    parser.add_argument(
        "spec_path",
